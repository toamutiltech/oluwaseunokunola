      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-16 text-center">
          ...
          <p className="text-slate-400">
            A comprehensive toolkit for modern, secure application delivery
          </p>
        </div>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {Object.entries(SKILLS).map(([category, items], i) => (
            <div
              key={category}
              className="glass-card p-8 rounded-2xl hover:border-blue-500/30 transition-colors group reveal-item reveal-hidden"
            ...
            >
              ...
              <div className="flex flex-wrap gap-2">
                {items.map((skill) => (
                  <span
                    key={skill}
                    className="px-3 py-1 bg-white/5 rounded-md text-sm text-slate-300 group-hover:bg-blue-500/10 transition-colors"
                  >
                    {skill}
                  </span>
                ))}
